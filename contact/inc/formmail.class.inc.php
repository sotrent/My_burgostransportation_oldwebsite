...
class Formmail
{
     ...
     */
    function check_value_limit($check_value, $count, $duration, $field, $err_msg, $filepath)
    {
        //global $filepath;

        if (!empty($count) and $count > 0) {
            if (is_file($filepath['logfile'] . 'logfile.txt')) {
                $check = '';
                $limit_errors_flag = '';

                $read_logfile = fopen($filepath['logfile'] . 'logfile.txt', "r");
...
//                }
     ...
     */
    function log_message($mail_content, $log_messages, $ip_address_count, $filepath, $txt, $tplt, $sender_count, $recipient_count)
    {
