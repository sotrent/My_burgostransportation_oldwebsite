...
if (!isset($system_message)) {
    if (isset($_POST['html_template'])) {
        $new_html_template = basename($_POST['html_template']);
    } else {
...
if (!isset($system_message)) {
    ...
    for ($i = 0; $i < count($mail_templates); $i++) {
        if (is_file($filepath['templates'] . basename(trim($mail_templates[$i])))) {
            $mail_content[] = join ('', file ($filepath['templates'] . basename(trim($mail_templates[$i]))));
        } else {
...
if (isset($_POST) and !empty($_POST)) {
    if ($check_ip = $mail->check_value_limit(getenv('REMOTE_ADDR'), $ip_address_count, $ip_address_duration, 0, $txt['txt_ip_address_expiration'], $filepath)) {
...
if (!isset($message) and empty($message) and isset($_POST) and !empty($_POST)) {
     ...
     */
    $mail->log_message($mail_status['mail_content'], $log_messages, $ip_address_count, $filepath, $txt, $tplt, $sender_count, $recipient_count);
